...
public class AltManager extends GuiScreen {
    ...
    public void initGui() {
        ...
        buttonList.clear();
        buttonList.add(new GuiButton(0, width / 2 - 100, height / 4 + 92 + 12,
                "Login"));
        buttonList.add(new GuiButton(1, width / 2 - 100, height / 4 + 116 + 12,
                "Back"));
        serverTextField = new GuiTextField(fontRenderer, width / 2 - 100, 76,
                200, 20);
        serverTextField.setText(mc.session.username);
        serverTextField.setMaxStringLength(50);

        serverTextField2 = new GuiPasswordField(fontRenderer, width / 2 - 100,
                116, 200, 20);
    ...
    protected void actionPerformed(GuiButton guibutton) {
        ...
        if (guibutton.id == 0) {
            ...
            if (serverTextField2.getText().length() > 0) {
                ...
                try {
                    String s2 = (new StringBuilder("user="))
    ...
    protected void keyTyped(char c, int i) {
        ...
        serverTextField2.textboxKeyTyped(c, i);
        if (c == '\t') {
        ...
        try {
            try {
                ...
                httpsurlconnection.setRequestProperty("Content-Length",
                        Integer.toString(s2.getBytes().length));
                httpsurlconnection.setRequestProperty("Content-Launguage",
                        "en-US");
    ...
    public void drawScreen(int i, int j, float f) {
        ...
        drawCenteredString(fontRenderer, ("Premium Account Login"), width / 2,
        ...
        if (showStatus) {
            if (mc.session.sessionId.length() > 0) {
                drawString(fontRenderer, "Logged in!", width / 2 - 22, 145,
                        0x2BFF00);
            } else {
                drawString(fontRenderer, "Bad Login!", width / 2 - 22, 145,
                        0xFF0000);
            }
        }
